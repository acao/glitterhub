...
export const initRelayEnvironment: InitRelayEnvironment = ({
  ...
  const network = Network.create(async ({ text: query }, variables) => {
    // const clientToken =
    //   !import.meta.env.SSR && window?.localStorage?.get('user.token')
    // const token = pageContext?.user?.token ?? clientToken

    const token = import.meta.env.GH_TOKEN ?? import.meta.env.VITE_GITHUB_TOKEN
    if (!token) {
